	...
	<div style="background: white; height: 200px; width: 280px; color: rgba(0, 0, 0, 0.7); padding-top: 30px;">
		...
		<div style="width: 100%; text-align:center; font-size: 25px; margin-top: 15px; margin-bottom: -10px;">
			<md-field class="time-input" :class="{'md-invalid': hourError}" :md-counter="false">
				<md-input @keydown.up="incrementHour()" @keydown.down="decrementHour()" v-model="hour" maxlength="2"></md-input>
			</md-field>
			:
			<md-field class="time-input" :class="{'md-invalid': minuteError}" :md-counter="false">
				<md-input @keydown.up="incrementMinute()" @keydown.down="decrementMinute()" v-model="minute" maxlength="2"></md-input>
			</md-field>
...
export default {
  ...
  methods: {
    ...
    set() {
      ...
      customTime.setHours(
        (+this.hour +
          12 * (+this.hour !== 12 ? this.ampm === "PM" : this.ampm === "AM")) %
          24
      );
    ...
    incrementMinute() {
      this.minute = (Number(this.minute) + 1) % 60;
      if (this.minute < 10) {
    ...
    incrementHour() {
      this.hour = Number(this.hour) % 12 + 1;
    },
    decrementHour() {
